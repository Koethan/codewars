...
function personalise( campaign, person){
    ...
    while(campaign.includes('<NAME>') || campaign.includes("<CITY>") || campaign.includes('<FAVOURITE PRODUCTS>')){
...
//    function personalise( campaign, person){
//     return campaign.split('<CITY>').join(person.city).split('<NAME>').join( person.name).split('<FAVOURITE PRODUCTS>').join(person.favourite_products)
//   }